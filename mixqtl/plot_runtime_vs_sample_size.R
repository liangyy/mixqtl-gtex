...
df_c = df_time %>% group_by(tissue) %>% summarize(nchr = n(), total_time_in_min = sum(runtime_in_min), avg_time_in_sec_per_gene = sum(runtime_in_min) / sum(npheno) * 60) %>% left_join(df_ss %>% select(tissue, v8_all), by = 'tissue')

p = df_c %>% filter(nchr == 22) %>% ggplot() + 
  geom_point(aes(x = v8_all, y = avg_time_in_sec_per_gene, color = tissue), size = 5, alpha = 0.5) + 

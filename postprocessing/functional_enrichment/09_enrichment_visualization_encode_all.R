...
df_combine = list()
for(tt in tissues) {
  df = read.csv(paste0('enrichment/encode_all_enrichment_', tt, '.csv'))
...
df_test = df_test %>% mutate(qtl_category_rename = rename_col(qtl_category, rename_map))
p2 = df_test %>% filter(category %in% include_func, qtl_category %in% to_show) %>% ggplot() + 
  ...
  ylab('Odds ratio')
ggsave('functional_enrichment_encode_all_by_tissue.png', p2, width = 7, height = 7)
...
ggsave('functional_enrichment_encode_all.png', p, width = 7, height = 5)
